		...
		for (String argument: args) {
			...
			if (options.containsKey(keyValue[0])) {
				switch (((CommandLineOption<?>)options.get(keyValue[0])).getOptionType()) {
					...
					case STRING:
						CommandLineOption<File> stringOption = (CommandLineOption<File>)options.get(keyValue[0]);
						if (keyValue.length == 2) {
							stringOption.setValue(new File(keyValue[1]));
						}
						else {
							parseErrors.add("Option should have a key and a value.");
						}
						break;
					case INTEGER:
						CommandLineOption<File> intOption = (CommandLineOption<File>)options.get(keyValue[0]);
						if (keyValue.length == 2) {
							intOption.setValue(new File(keyValue[1]));
						}
						else {
							parseErrors.add("Option should have a key and a value.");
						}
